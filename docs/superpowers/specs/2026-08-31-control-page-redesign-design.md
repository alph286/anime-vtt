...
- **Trascinamento dell'inquadratura, dietro un interruttore esplicito**: un nuovo pulsante (spento di default — funzione usata raramente) attiva/disattiva la "modalità sposta inquadratura" sull'anteprima mappa.
  - **Spento** (sempre, salvo attivazione esplicita): il tocco sulla mappa si comporta esattamente come oggi — solo rivela/nasconde la zona fog toccata. Il rettangolo resta puramente visivo, nessuna interpretazione di trascinamento.
  - **Acceso**: il tocco sul fog viene sospeso del tutto sull'anteprima mappa (nessuna ambiguità tap-vs-trascinamento da risolvere: mentre il pulsante è attivo ogni gesto è per forza un trascinamento). Trascinare in un punto qualsiasi dell'anteprima — non serve centrare precisamente il rettangolo, che può essere sottile o coincidere con l'intera anteprima a zoom 1 — sposta l'inquadratura live in tempo reale, con feedback visivo immediato sul rettangolo mentre si trascina.
  - Il pulsante resta acceso finché non lo si disattiva esplicitamente (nessuno spegnimento automatico a fine gesto).
  - Se `displayViewport` non è disponibile (il display non si è mai connesso, niente rettangolo da mostrare), il pulsante resta disabilitato — stesso principio già usato altrove nell'app (es. `LOCATION_DEPENDENT_CONTROLS` nell'editor) per i controlli senza senso in quel momento. Il pad direzionale resta comunque sempre disponibile come alternativa, sia per le regolazioni fini sia quando il trascinamento non è disponibile.
  - Tecnicamente: durante il trascinamento (pointerdown/pointermove/pointerup sull'anteprima, stesso pattern già usato in `editor.js` per il trascinamento dei vertici e del riquadro di allineamento griglia), i delta in pixel dell'anteprima locale si convertono in delta nello spazio pixel della TV usando lo stesso rapporto di scala derivato da `displayViewport` per disegnare il rettangolo, ed emettono `view:pan` in tempo reale (throttled, es. per frame) durante il gesto.
